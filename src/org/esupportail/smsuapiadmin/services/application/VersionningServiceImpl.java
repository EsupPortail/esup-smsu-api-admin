 ...
 */
package org.esupportail.smsuapiadmin.services.application;
 ...
 */
public class VersionningServiceImpl extends AbstractDomainAwareBean implements
		VersionningService {
	...
	public void afterPropertiesSetInternal() {
		Assert.notNull(this.firstAdministratorId,
				"property firstAdministratorId of class "
	...
	public void initDatabase() {
		...
		//User firstAdministrator = getDomainService().getUser(
	...
	/**
	 * @see org.esupportail.commons.services.application.VersionningService#checkVersion(boolean,
	 *      boolean)
	 */
	public void checkVersion(final boolean throwException,
		...
		if (applicationVersion.isSameMajorAndMinor(databaseVersion)) {
			logger.info("Database version is " + databaseVersion
					+ ", upgrading...");
			upgradeDatabase();
			if (printLatestVersion) {
				printLastVersion();
		...
		String msg = "The application is too old (" + databaseVersion
				+ "), please upgrade.";
		if (throwException) {
			throw new VersionException(msg);
		}
	...
	protected void printOlderThanMessage(final String version) {
		logger.info(new StringBuffer("database version (").append(
				getDomainService().getDatabaseVersion()).append(
				") is older than ").append(version).append(", upgrading..."));
	}
	...
	private void upgradeDatabaseIfNeeded(final String version) {
		...
		try {
			...
			throw new ConfigException("access to the information of class "
					+ getClass() + " was denied", e);
		} catch (NoSuchMethodException e) {
			throw new ConfigException("could no find method " + getClass()
					+ "." + methodName + "()", e);
		}
		...
		throw new ConfigException("could no invoke method " + getClass() + "."
				+ methodName + "()", invocationException);
	}
	...
	public boolean upgradeDatabase() {
		...
		if (!getDatabaseVersion().equals(getApplicationService().getVersion())) {
			setDatabaseVersion(getApplicationService().getVersion().toString(),
					false);
		}
		return false;
	}
	 ...
	 *            the firstAdministratorId to set
	 */
